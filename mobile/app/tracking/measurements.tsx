...
export default function BodyMeasurements() {
  const [measurements] = useState<Measurement[]>([
    ...
    { bodyPart: 'Hombros', current: 118, previous: 116, change: 2.0, unit: 'cm', icon: 'üí™', color: 'primary' },
    ...
    { bodyPart: 'Cadera', current: 98, previous: 99, change: -1.0, unit: 'cm', icon: 'üçë', color: 'primary' },
    { bodyPart: 'BÌcep Izq', current: 38.5, previous: 37.5, change: 1.0, unit: 'cm', icon: 'üí™', color: 'blue' },
    { bodyPart: 'BÌcep Der', current: 39.0, previous: 38.0, change: 1.0, unit: 'cm', icon: 'üí™', color: 'blue' },
    ...
    { bodyPart: 'Muslo Der', current: 58.5, previous: 56.5, change: 2.0, unit: 'cm', icon: '??', color: 'primary' },
  ...
  const addMeasurement = () => {
    Alert.alert(
      'Nueva MediciÛn',
      'FunciÛn para registrar todas las medidas corporales con fecha',
      [{ text: 'Entendido' }]
    );
  };
  ...
  const viewHistory = (bodyPart: string) => {
    Alert.alert(
      `Historial: ${bodyPart}`,
      'Gr·fico de evoluciÛn en el tiempo',
      [{ text: 'Cerrar' }]
    );
  };
  ...
  const getAsymmetry = () => {
    const bicepDiff = Math.abs(measurements.find(m => m.bodyPart === 'BÌcep Izq')!.current - 
                                measurements.find(m => m.bodyPart === 'BÌcep Der')!.current);
    ...
    <View className="flex-1 bg-zinc-950">
        ...
        <View className="px-6 pt-6">
          ...
          <View className="flex-row gap-2 mb-6">
            <View className="flex-1 bg-primary/10 rounded-xl p-3 border border-primary/30">
              ...
              <Text className="text-primary/80 text-xs">Total m˙sculo</Text>
            </View>
            <View className="flex-1 bg-primary/10 rounded-xl p-3 border border-primary/30">
              <Text className="text-primary/80 text-xs mb-1">P…RDIDA</Text>
              ...
              <View className="flex-row items-start">
                ...
                <View className="flex-1 ml-3">
                  <Text className="text-amber-400 font-bold mb-2">
                    AsimetrÌas Detectadas
                  </Text>
                  <View className="space-y-1">
                    {asymmetry.bicep > 0.5 && (
                      <Text className="text-amber-300 text-sm">
                        ï BÌceps: {asymmetry.bicep.toFixed(1)} cm diferencia
                      </Text>
                    )}
                    {asymmetry.thigh > 0.5 && (
                      <Text className="text-amber-300 text-sm">
                        ï Muslos: {asymmetry.thigh.toFixed(1)} cm diferencia
                      </Text>
                    )}
                    {asymmetry.calf > 0.5 && (
                      <Text className="text-amber-300 text-sm">
                        ï Gemelos: {asymmetry.calf.toFixed(1)} cm diferencia
                      </Text>
                    )}
                  </View>
                </View>
          ...
          <View className="bg-zinc-900 rounded-xl p-6 mb-6 border border-zinc-800">
            <Text className="text-white text-lg font-bold mb-4">GuÌa de MediciÛn</Text>
            
            <View className="space-y-3">
              <View>
                <Text className="text-primary font-bold mb-1">? Mismo horario</Text>
                <Text className="text-zinc-400 text-sm">
                  Mide siempre a la misma hora (ej: maÒana en ayunas)
                </Text>
              </View>

              <View>
                <Text className="text-primary font-bold mb-1">? Cinta mÈtrica flexible</Text>
                <Text className="text-zinc-400 text-sm">
              ...
              <View>
                <Text className="text-primary font-bold mb-1">? M˙sculos relajados</Text>
                <Text className="text-zinc-400 text-sm">
                  No flexiones (excepto para medida de bÌceps contraÌdo)
                </Text>
              </View>
      ...
      <View className="px-6 pb-6 pt-4 border-t border-zinc-800">
        ...
        >
          ...
          <Text className="text-white font-bold ml-2">Registrar Nueva MediciÛn</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
