          ...
          {filteredExercises.map((exercise) => (
            <TouchableOpacity
              ...
              className={`bg-zinc-900 rounded-xl p-4 mb-4 border-2 ${
                selectedExercise === exercise.id ? 'border-emerald-500' : 'border-zinc-800'
              }`}
            >
              {/* Header */}
              ...
              <View className="bg-zinc-800 rounded-lg p-3 mb-3">
                ...
                <View className="flex-row flex-wrap gap-2">
                  {exercise.muscleGroup.map((muscle, index) => (
                    <View key={index} className="bg-emerald-500/20 px-3 py-1 rounded-full">
                      <Text className="text-emerald-400 text-xs font-bold">{muscle}</Text>
                    </View>
                  ))}
                </View>
              </View>
              ...
              <View className="bg-blue-500/10 rounded-lg p-3 border border-blue-500/30 mb-3">
                ...
                {exercise.instructions.slice(0, 3).map((instruction, index) => (
                  <View key={index} className="flex-row items-start mb-1">
                    <Text className="text-blue-400 mr-2">{index + 1}.</Text>
                    <Text className="text-blue-300 text-sm flex-1">{instruction}</Text>
                  </View>
                ))}
